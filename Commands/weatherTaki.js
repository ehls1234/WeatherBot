module.exports = {
    ...
    execute(message,args){
        ...
        let nowdate = DateTime.now().plus({ days: 1 }).day
        nowdate = ("0" + nowdate.toString()).slice(-2)
        ...
        .addField("사용법","!내일날씨 <시/도> <선택:시/구/군> <선택:읍/면/동>"))
        fs.readFile("./Data.json",function(err,result){
              ...
              for(var x = 0; x < jsondata.response.body.items.item.length; x++){
                  switch(jsondata.response.body.items.item[x].category){
                    case "SKY":
                        ...
                        case "1":
                            SKY = "내일은 구름하나 없이 좋은 날이에요!"
                            break
                        case "3":
                            SKY = "내일은 구름이 조금 많은 날이네요."
                            break
                        case "4":
                            SKY = "내일은 흐린 날이에요."
                            break
                    ...
                    case "PTY":
                        switch(jsondata.response.body.items.item[x].fcstValue){
                            case "1":
                                PTY = "내일은 비가 내릴 수도 있어요, 외출시 우산을 챙겨야겠네요."
                                break
                            case "2":
                                PTY = "내일은 비와 눈이 섞여서 올 수 있어요, 외출시 우산을 챙겨야겠네요."
                                break
                            case "3":
                                PTY = "내일은 새하얀 눈이 내릴 예정이에요"
                                break
                            case "4":
                                PTY = "내일은 소나기가 내릴 예정이에요, 외출 시 우산을 꼭 챙겨야겠어요."
                                break
                            case "5":
                                PTY = "내일은 빗방울이 조금씩 내릴 에정이에요, 외출 시 우산을 챙겨야겠어요."
                                break
                            case "6":
                                PTY = "내일은 빗방울이나 눈이 조금씩 날릴 예정이에요"
                                break
                            case "7":
                                PTY = "내일은 눈이 조금씩 날릴 예정이에요"
                                break
                        }
                        break
                        case "POP":
            ...
            var description = ""
            var embed = new Discord.MessageEmbed().setTitle("내일의 날씨")
            if(POP){
                if(!typeA && !typeB){
                    embed.addFields({name:"내일의 하늘",value:SKY},{name:"내일의 습도",value:REH+"%"},{name:"아침 최저기온",value:TMN},{name:"낮 최고기온",value:TMX},{name:"내일의 강수 형태",value:PTY})
                ...
                if(!typeA && !typeB){
                    embed.addFields({name:"내일의 하늘",value:SKY},{name:"내일의 습도",value:REH+"%"},{name:"아침 최저기온",value:TMN},{name:"낮 최고기온",value:TMX})
                    description = json[i][0]+"의 "
                }else if(typeA){
                    embed.addFields({name:"내일의 하늘",value:SKY},{name:"내일의 습도",value:REH+"%"},{name:"아침 최저기온",value:TMN},{name:"낮 최고기온",value:TMX})
                    description = json[i][0]+" "+json[i][1]+"의 "
